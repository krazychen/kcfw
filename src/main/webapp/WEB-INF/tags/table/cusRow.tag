...
<button class="btn btn-white btn-sm" data-toggle="tooltip"
	data-placement="left" onclick="cus${id}()" title="${label}">
	<i class="fa fa-file-text-o"></i> ${label==null?'自定义':label}
</button>
</button>
...
$(document).ready(function() {
	...
	function cus${id}(){
		...
		openDialogCus('${title}',"${url}?id="+id,"${width==null?'800px':width}", "${height==null?'500px':height}","${target}");
		</c:if>
		<c:if test="${ empty id}">
		openDialogCus('${title}',"${url}","${width==null?'800px':width}", "${height==null?'500px':height}","${target}");
		</c:if>
	}
</script>