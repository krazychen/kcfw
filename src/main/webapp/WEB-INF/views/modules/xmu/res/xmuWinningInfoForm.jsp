			...
			top.layer.open({
			    ...
			    yes: function(index, layero){
					 ...
					 if(items=="-1"){
						 ...
						 $("#xwiUserId").val(items[0]);
						 $("#xwiUserStuno").val(items[1]);
						 $("#xwiUserName").val(items[2]);
						 $("#xwiOfficeId").val(items[5]);
						 $("#xwiOfficeName").val(items[6]);
						 $("#xwiUserProfession").find("option[value='"+items[4]+"']").attr("selected",true);
						// $("#xaeUserProfession").val(items[4]);
						 $("#xwiUserGrade").val(items[3]);
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
					...
					<td class="width-35">
						...
						<c:if test="${fn:contains(role, 'Manager')}" >
							<div class="input-group">
								<input id="xwiUserStuno"  name="xwiUserStuno" readonly="readonly" type="text" value="${xmuWinningInfo.xwiUserStuno}" class="form-control "/>
					       		 <span class="input-group-btn">
						       		 <button type="button" onclick="addStu();" id="xwiUserStunoButton" class="btn btn-primary"><i class="fa fa-search"></i>
						             </button> 
					       		 </span>
				...
				<tr>
					...
					<td class="width-35">
						<input id="xwiWinningYears" name="xwiWinningYears" type="text" maxlength="20" class="laydate-icon form-control layer-date formDateMaxWidth "
							value="${xmuWinningInfo.xwiWinningYears}"/>
					</td>
					<td class="width-15 active"><label class="pull-right"><font color="red">*</font>竞赛名称：</label></td>
					<td class="width-35">
						<form:input path="xwiWinningName" htmlEscape="false" maxlength="200" class="form-control required"/>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">竞赛级别：</label></td>
					<td class="width-35">
						<form:select path="xwiWinningLevel" class="form-control ">
							<form:option value="" label=""/>
