		...
		<div class="pull-left">
			<c:if test="${!fn:contains(role, 'dept')}" >
				<shiro:hasPermission name="xmu:res:xmuAcademicEvent:add">
					<table:addRow url="${ctx}/xmu/res/xmuAcademicEvent/form" stu="${stu}" title="学术活动"></table:addRow><!-- 增加按钮 -->
				</shiro:hasPermission>
				<shiro:hasPermission name="xmu:res:xmuAcademicEvent:edit">
			    	<table:editRow url="${ctx}/xmu/res/xmuAcademicEvent/form" stu="${stu}" status="1" title="学术活动" id="contentTable"></table:editRow><!-- 编辑按钮 -->
				</shiro:hasPermission>
				<shiro:hasPermission name="xmu:res:xmuAcademicEvent:submit">
			   		<table:submitRow url="${ctx}/xmu/res/xmuAcademicEvent/form" stu="${stu}" title="学术活动" id="contentTable"></table:submitRow><!-- 提交按钮 -->
				</shiro:hasPermission>
			</c:if>
			<c:if test="${fn:contains(role, 'Manager')}" >
				<shiro:hasPermission name="xmu:res:xmuAcademicEvent:audit">
				    <table:auditRow url="${ctx}/xmu/res/xmuAcademicEvent/form" status="2" targetAction="${ctx}/xmu/res/xmuAcademicEvent/saveAudit" title="学术活动" id="contentTable"></table:auditRow><!-- 审核按钮 -->
				</shiro:hasPermission>
			</c:if>
			<c:if test="${fn:contains(role, 'dept')}" >
				<shiro:hasPermission name="xmu:res:xmuAcademicEvent:audit">
				    <table:auditRow url="${ctx}/xmu/res/xmuAcademicEvent/form" status="3" targetAction="${ctx}/xmu/res/xmuAcademicEvent/saveAudit" title="学术活动" id="contentTable"></table:auditRow><!-- 审核按钮 -->
				</shiro:hasPermission>
			</c:if>
			
			<c:if test="${fn:contains(role, 'Student')}" >
				<shiro:hasPermission name="xmu:res:xmuAcademicEvent:back">
				    <table:backRow url="${ctx}/xmu/res/xmuAcademicEvent/form" stu="${stu}" status="2" targetAction="${ctx}/xmu/res/xmuAcademicEvent/back" title="学术活动" id="contentTable"></table:backRow><!-- 撤回按钮 -->
				</shiro:hasPermission>
			</c:if>
			<c:if test="${fn:contains(role, 'Manager')}" >
				<shiro:hasPermission name="xmu:res:xmuAcademicEvent:back">
